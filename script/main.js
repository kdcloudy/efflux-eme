  ...
  .then(function (keySystemAccess) {
    ...
    promise
      ...
      .catch(function (error) {
        console.error(
          "Unable to set MediaKeys on " + "video object : " + error
        );
      });
  })
  .catch(function (error) {
...
function playMedia() {
  ...
  try {
    ...
    console.error(
      "Error while adding an event listener for 'encrypted' event on video element. Error: " +
        err.message
    );
  }
  html5video.play();
...
function handleEncrypted(e) {
  ...
  try {
    ...
    console.error(
      "Unable to add 'message' " +
        "event listener to the keySession object. Error: " +
        err.message
    );
  }
  ...
  promise.catch(function (error) {
    console.error(
      "Unable to create or " + "initialize key session. Error : " + error
    );
  });
}
